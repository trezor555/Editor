#include "SkinnedMeshEvaluator.h"
...
void AnimEvaluator::Evaluate(double pTime)
{
	...
	for (unsigned int a = 0; a < mAnim->mNumChannels; a++)
	{
		...
		if (channel->mNumPositionKeys > 0)
		{
			...
			unsigned int frame = (time >= mLastTime) ? mLastPositions[a].x : 0;
		...
		{
			unsigned int frame = (time >= mLastTime) ? mLastPositions[a].y : 0;
		...
		{
			unsigned int frame = (time >= mLastTime) ? mLastPositions[a].z : 0;
